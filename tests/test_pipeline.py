...
def test_run_produces_nquads_and_report(seeded_lake, tmp_path):
    ...
    assert report.organizations == 27       # 入力の全件数(現行26機関+株式会社1件)
    assert report.government_organs == 26   # KGに入れた件数(株式会社1件を除外)
...
def test_run_reports_unmatched_ministries(seeded_lake, tmp_path):
    ...
    裁定B12で参照表は26行(RS実データの所管府省庁名23件 + 法令経路3機関)に
    拡張された。`houjin_bangou_sample.csv` もこの26機関すべてを実在の法人番号で
    含む(R45)ので、正常系では突合率100%になる。ここを厳密に固定することで、
    突合が壊れたときに検出できる。
    """
    report = pipeline.run(FETCHED, tmp_path / "out")
    assert report.ministries == 26, "参照表の26機関すべてが突合されるべき"
