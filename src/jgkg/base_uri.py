...
SOURCE_GLOBS: tuple[str, ...] = (
    ...
    # `site/def/`配下(生成物のコピー)はこのglobが非再帰なので対象にならず、
    # `schema/generated/*.ttl`側の検査と二重にはならない(`site/def/`自体は
    # 要修正1より前から生成物だった)。`site/_headers`は要修正1で生成物に
    # なったが、内容は`/def/core`のような相対パスのみでドメイン文字列を
    # 一切含まない(実測。旧来の手書きワイルドカードの頃も同様)——非再帰
    # だから安全という話ではなく、そもそも書き換え対象が無いので素通りする。
    "site/*",
)
...
# 全角の `（）` も除外する(B-1: `sources.py` の `citation` に、政府サイト自身の
# 出典記載例と同じ書式「（URL）」を採ったところ、半角除外だけでは全角の閉じ
# 括弧がURLの一部として誤って取り込まれ、ホストが `rssystem.go.jp）` のように
# 化けて検出された)
