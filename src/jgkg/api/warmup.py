...
**温めはコストを消さない、起動時に移すだけである(裁定B61)。**
「これで初回リクエストが速くなる」とは言わない。正確には
「エンドポイントが実際に走る領域を、コード経路から導出して温める」
までである。

**開発環境の初回読みの数字を配備先の代表値として使わない(裁定B62)。**
開発環境の索引はWindowsバインドマウント上にあり、コールドな初回読みが
極端に遅く見える——仕様§6.3が「TDB2はmmapを使うのでネットワークファイル
共有に置くな」と警告した置き方そのもので、`docs/measurements-phase1.md`
の発見7がその原因(バインドマウント越しのランダムI/O)を実測している。
**配備先の構成での初回読みは未実測——D-6で測る。** 索引の置き場所
(バインドマウント越しか、実ローカルFS/イメージ焼き込みか)という
プラットフォーム側の解決もD-6の課題(進行台帳B62参照)。
...
#: 全走査される。**さらに、空文字列は`_build_search_query`が持つ5つの
#: OPTIONAL(`law:lawNum`・`org:prefectureName`・`org:cityName`・
#: `budget:fiscalYear`・`budget:ministry/skos:prefLabel`)も全件分温める**
#: ——これらは`/search`の要約導出(`_derive_summary`)と`/entity/{id}`が
#: 実際に読む領域である(裁定B67がこの選択を実データで検証済み。
#: controllerは当初「無駄なコストではないか」と疑ったが、実測の結果
#: 変更を要求しなかった)。
_WARMUP_SEARCH_TERM = ""
