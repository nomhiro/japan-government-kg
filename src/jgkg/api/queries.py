...
def _id_path(base_uri: str, iri: str) -> str:
    ...
    参照)の逆演算。**この関数1本だけを、`id`を組み立てるすべての箇所
    (`SearchHit`・関係の相手側の`EntityRef`・`EntityDetailResponse`、
    さらにモジュール外の`overview.MinistryBudget`)から呼ぶ**(裁定B59)。
    箇所ごとに剥がし処理を手書きすると、このプロジェクトの再発欠陥1
    (導出すべき値を手書きしている)そのものになる。

    **呼び出し箇所の個数はここに書かない(訂正。レビュー要修正10)。**
    以前この節は呼び出し箇所を「3つ」と数えて書いていたが、これは
    古くなった数字だった——箇所が増えるたびに更新が必要になり、更新を
    怠ると偽の主張が残る(この関数はその後、モジュール外の`overview.py`
    からも呼ばれるようになった)。**数えて書くのではなく、「idを組み立てる
    すべての箇所から呼ぶ」という規律だけを書く**——具体的にどこから呼ばれて
    いるかは、この関数名で検索すれば常に正確に分かる。
