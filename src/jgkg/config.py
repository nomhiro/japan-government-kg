...
class Settings(BaseSettings):
    ...
    # =========================================================================
    # 裁定B103追記3: lifespanでトリプルストアの応答を待つ(上限付き)。
    # =========================================================================
    #
    # **なぜ要るか**: `src/jgkg/api/readiness.py`のモジュールdocstring参照
    # (ACAには同一レプリカ内のコンテナ起動順序を指定する仕組みが無く、
    # Fusekiが先に応答できる前提が崩れると`/overview`が永久に503になる)。
    #
    #: **根拠(実測)**: 裁定B76がFuseki自身の起動から最初の応答までを
    #: 実Linux FS上で**1.242秒**と実測している(索引のページインを含まない。
    #: `docs/decision-log.md`)。team-lead自身の実測も「Fusekiの再起動から
    #: 応答までは数秒」——この既定値(30秒)はそれに約24倍の安全マージンを
    #: 取った値であり、**本番配備でのコンテナ間の実際の遅れそのものの実測
    #: ではない**(Task 7が実測する予定。`docs/decision-log.md`の裁定
    #: 「上限の値は実測で決める」参照)。短すぎると配備の遅い環境で間に合わず、
    #: 長すぎるとFusekiが本当に落ちている場合の起動全体を無用に延ばす
    #: (`lifespan`はこの待ちの後に`warm_up`/`build_overview`を置くため、
    #: 他の全エンドポイントもこの待ちが終わるまで応答できない——B103の
    #: 既存の設計〔起動時にブロッキングで集約する〕をそのまま延長した
    #: トレードオフである)。
    overview_readiness_timeout_seconds: float = 30.0
    #: 上の上限に達するまでの再試行間隔(秒)。**根拠が無い**——0.5秒刻みは
    #: 「1.242秒という観測値の半分以下で、かつビジーループにならない」という
    #: 構造的な選択であり、実測に基づく値ではない(`fusekiCpu`等と同じ
    #: 「実測していないことを明記する」作法)。
    overview_readiness_poll_interval_seconds: float = 0.5
