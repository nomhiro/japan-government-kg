...
## D-4の実データ検証(controller。2026-08-30。利用者と同じ入り口=TestClient経由)

**裁定B72の(3)で繰り越した検証を実施した。** ポート3030が空いたため
(別プロジェクトのFusekiが終了。観察O16)、我々のFusekiを起動して行った。

**測定前にデータセットの同一性を確認した**(観察O16の統制):

```
クアッド数: 884,052(期待 884,052)
名前付きグラフ: 7(期待 7)
```
...
```
warm_up(): 142.234 秒(ラベル領域がコールドだった1回目)
           1.125 秒(2回目以降)
```

**先に流した `COUNT(*)` は1.860秒で終わったが、それは別の索引領域である**
——**「接続できて速かった」は「ラベル領域が温まっている」ではない。**
裁定B60が示した領域の違いが、そのまま出た。

**この142秒は開発環境(Windowsバインドマウント)の数字であり、
配備先の代表値ではない**(裁定B62)。配備先の測定は裁定B71の方法(CI)で行う。

### 近傍サブグラフ(中心=厚生労働省。実データで最大級のハブ)

| 深さ | 所要 | 逐次クエリ | ノード | エッジ | 打ち切り |
|---|---|---|---|---|---|
| 1 | 0.453秒 | **4本** | 26 | 25 | 分岐数の上限で1ノード |
| 2 | 1.125秒 | **18本** | 100 | 200 | **ノード数・エッジ数の両方** + 分岐数で4ノード |

深さ2のノードの型: `Expenditure` 67 / `BudgetProject` 25 / `Law` 6 /
`Ministry` 1 / `UnresolvedReference` 1。
述語: `project` 168 / `ministry` 25 / `basisLaw` 6 / `unresolvedFor` 1。

**すべての辺の `graph` が `graphs` に存在し、出典が空のグラフは0本。**

**1ノード1クエリの設計の代償が数字で出た**: 深さ2で**18本の逐次クエリ**。
温まった状態で1.125秒だが、**D-5が近傍展開のUIを作るときの待ち時間の
見積もりはこれである**(コールドなら1本あたりのコストが乗る)。

### 合成(裁定B59/B69と同じゲート)

**近傍のノード26個すべてについて `/entity/{id_path}` が200を返し、
`id` が一致した(26/26)。**

### パス探索(仕様§9.1が例に挙げた「法令↔法人」)

**実データで経路を見つけた**:

```
Law「高齢者の医療の確保に関する法律」        --basisLaw-->
BudgetProject「令和6年能登半島地震に係る医療保険者等への財政支援」 --project-->
Expenditure「栃木県」                       --recipient-->
Organization「栃木県」

found=True / 経路長3 / 訪問136 / 探索深さ3 / 0.969秒
budget_exhausted=False depth_limited=False fanout_truncated=True exhaustive=False
**逆向きに辿った辺: 2 本**
```

**これは設計書§1.2(C)の縦スライスそのものを、辺の向きに2回逆らって
辿った結果である。** 向きを守る探索ではこの経路は見つからない
(`undirected=True` を応答に出す根拠が実データで確認できた)。

**正直さの欄も実データで働いた**: 最初に選んだ法令(予算事業が紐づいて
いなかった)では `found=False` かつ `depth_limited=True` /
`fanout_truncated=True` / `exhaustive=False` を返した ——
**「無い」ではなく「見つからなかった」と正しく報告している。**

### `%` を含むノード(厚生省。`AbolishedGovernmentOrgan`)

```
近傍 status=200 / ノード 26 / エッジ 25
center id: .../id/org/abolished/%E5%8E%9A%E7%94%9F%E7%9C%81(正しくエンコードされた形)
ノードの型: {AbolishedGovernmentOrgan: 1, Law: 25}
述語: {jurisdiction: 25}
```

## 裁定 B74: 分岐数の上限が「意味的に最も重要な辺」を隠すことがある(D-5への入力)

**厚生省の近傍に `succeededBy`(承継先=厚生労働省)が現れなかった。**

原因: 分岐数の上限(既定25)に達し、`_build_expansion_query` の
`ORDER BY ?direction ?p ?other` の順序で **`incoming` の `jurisdiction` 25本が
先に埋めた**ため、`outgoing` の `succeededBy` が切られた。

**切ったことは正しく報告されている**(`fanout_truncated_nodes` に厚生省が
入る)。**欠陥ではない。** しかし:

**Cブロックの成果(旧省庁の承継。解決率65.27%→95.77%)を辿るための辺が、
法令25本の陰に隠れる。** 利用者が厚生省の近傍を見て「承継先が分からない」
と読む可能性がある。

### なぜ今は直さないか

**上限を上げるのは解決ではない**(ハブでは必ずどこかで切れる)。
**必要なのは「どの辺を優先して見せるか」の判断**であり、それは
**表示の問題(D-5)** である ——
仕様§9.2「エンティティページ(型別レイアウト)」が、型ごとに何を
どの順で見せるかを決める場所である。

### 裁定

**(1) API側の既定の順序は変えない。** `ORDER BY ?direction ?p ?other` は
**決定的**(同じ入力に同じ結果)であり、それ自体は正しい性質である。
**順序を「意味の重要度」で決めるのは、APIが表示の判断を持つことになる。**

**(2) D-5のブリーフに入れる**: 近傍を展開するUIは、
**分岐数の上限で切られたノードがあるとき、それを利用者に見せ、
述語を選んで展開できる経路を用意すること。**
`fanout_truncated_nodes` がその情報を既に返している。

**(3) 併せてD-5に渡す実測**: 深さ2の近傍は**18本の逐次クエリ**で
温まった状態1.125秒。**UIが「展開」を押した直後に何も出ない時間がある。**
段階的に出すか、深さ1を既定にして明示的に深めさせるかは表示の判断。

## 裁定 B76: 配備先の初回読みを実測した。B55/B61/B62の未決が解けた

**裁定B55・B61・B62が3回繰り越し、裁定B71が方法を定めた数字が出た。**
2026-08-30、`cold-read-measurement.yml`(`workflow_dispatch`)を実行。

### 実測(GitHub Actions ubuntu-latest。真にコールドな実Linux FS=ext4)

対象: 公開リリース `2026-08-27-c3-succession-v3`(**810,133クアッド** / 7グラフ。
展開後 449 MB)。**manifestと突き合わせて同一性を確認**し、
**各段が0件でないことも確認**した(裁定B75の空虚検出が通った)。

| 段 | 1回目(コールド) | 2回目 | 3回目 |
|---|---|---|---|
| Fusekiが応答を返すまで(索引のページインを含まない) | **1.242 秒** | — | — |
| **ラベル領域**(`warm_up` = APIの初回リクエスト) | **2.948 秒** | 1.247 秒 | 1.191 秒 |
| **支出領域**(§19.3が175.250秒を測った領域) | **1.807 秒** | 1.101 秒 | 0.983 秒 |
| エンティティ詳細(1エンティティ分) | **0.035 秒** | 0.027 秒 | 0.023 秒 |

### 開発機(Windowsバインドマウント)との比較

| 領域 | バインドマウント | 実Linux FS | 比 |
|---|---|---|---|
| ラベル | **142.234秒**(§20) | **2.948秒** | 約48× |
| 支出 | 175.250秒(§19.3)/ 121.437秒(2026-08-30再測) | **1.807秒** | 約67〜97× |

**注意**: 索引の中身が完全に同じではない(バインドマウント側は884,052クアッド、
ランナー側は810,133クアッド)。**厳密なA/Bではないが、桁の差はその違いでは
説明できない。**

### 3つの未決が解ける

**(1) 裁定B61の懸念(「温めが起動時に111〜142秒かかり起動プローブを超える」)は
実Linux FSでは成立しない。** **温めは約3秒である。**

**(2) 裁定B62が撤回した外挿(「58倍だから配備先なら数秒」)は、
方向も桁も結果的に正しかった。** **しかし撤回は正しかった** ——
あの時点で根拠が無かったからである(発見7は書き込みの測定だった)。
**正しい結論に、誤った根拠で先に到達しても、それは正しい推論ではない。**

**(3) 裁定B55の表(tmpfs 対 実ディスク)を実測で置き換える。**
...
**「初回に最大175秒」はWindowsバインドマウントの数字だった。**
実測に置き換えると:

| 一時領域 | メモリ | 初回クエリ(実測) |
|---|---|---|
| tmpfs | 実測 + 428.5 MiB | 0秒に近い(未実測。理屈上ページフォルトが無い) |
| **実Linux FS** | **追加なし** | **約1.2〜2.9秒(実測)** |

**tmpfsに +428.5 MiB のメモリを払って買えるのは約2秒である。**
**B55が示唆した「プラットフォームの判断が逆転しうる」は、
実測では逆転しない** —— 実Linux FSに置けば十分である。

### コールド/ウォーム比についての訂正

**裁定B55は「200倍の差。原因はページフォルトである」と書いた。**
実Linux FSでの比は **約2.5倍**(2.948 → 1.191)である。

**200倍はバインドマウントの性質であって、TDB2のmmapの性質ではなかった。**
B55の「原因はページフォルトである」という説明は、
より正確には「**ネットワークファイル共有相当の経路越しのページフォルト**」
だった(裁定B62が指摘した方向を、実測が確定させた)。

### D-6への結論

**(1) 索引は実Linuxファイルシステム(コンテナのローカルディスク)に置く。**
バインドマウント越しの配信は選択肢から外す(仕様§6.3の要求であり、
実測でも48〜97倍の差がある)。

**(2) tmpfsは不要。** +428.5 MiB のメモリで約2秒を買う取引になる。

**(3) 温めは維持する。** 約3秒を起動時に払って、最初の利用者から外す。
**起動プローブの危険物ではない** —— B61の懸念は実Linux FSでは消える。

**(4) D-1の「コールドスタート14秒」との関係**: ランナーでの
「Fusekiが応答を返すまで」は **1.242秒**。D-1の14秒には成果物の
ダウンロード・展開・照合が含まれていた(裁定B55が「索引のページインを
含んでいなかった」と指摘した数字)。**段を分けて測ったので、
今回はどちらも混ざっていない。**
...
- 対象は**810,133クアッドの公開リリース**である。884,052クアッドの
  D-2成果物(未公開。裁定B68)では測っていない
- **1回の実行である。** ランナーの個体差は測っていない
  (裁定B55の「1回で結論しない」は**同じ実行内での反復**には従ったが、
  **実行間の反復**は行っていない)

## 裁定 B77: D-4のタスクレビューがダングリングエッジ欠陥を実証した(新種)

**判定: 仕様適合 ✅ / タスク品質 要修正 → 修正済み。**

レビュアーが独自に `pytest`(728件)・`ruff`・`base_uri --check` を実行して
確認した上で、**controllerが見落とした欠陥を実証した。**

### 欠陥: `node_limit` 到達時にダングリングエッジが残る

`get_neighborhood` は辺を `hops` に確定した**後**で相手ノードの `node_limit` を
判定していた:

```python
hops[hop] = None          # ← 辺を先に確定
other = hop.other_than(node)
if other in seen_nodes: continue
if len(seen_nodes) >= node_limit:
    nodes_truncated = True
    continue              # ← ノードは除外。しかし辺は既に入っている
```

**結果: `nodes` に無いノードを指す辺が応答に残る。**
レビュアーの実証(ハブ6辺・`node_limit=2`): nodes 2件に対して edges 6件、
**6件中5件がダングリング。**

**`nodes_truncated` は正直に真を返していた** ——
**打ち切りの報告は正しいのに、返したグラフ自体が壊れていた。**
**D-5が `edges.source`/`target` は常に `nodes` にある前提で描画すると破綻する。**

### これは既存の9型に一致しない新種である(レビュアーの指摘)

**型10: 打ち切りを正直に報告しているのに、返した結果の構造自体が壊れている。**

既存の型6(報告が嘘をつく)とは違う ——
**報告は嘘をついていない。データ構造が不整合なだけである。**
「正直さ」の検査(`truncated` フラグが立つか)を通しても、
**構造の整合性(端点が集合に含まれるか)は別に検査しなければならない。**

### controllerのテストが検出できなかった理由

`test_neighborhood_node_limit_appears_in_the_response` は
`len(nodes) <= 2` と `nodes_truncated is True` しか見ていなかった。
**アサーションは正しいが、検査していない軸があった**(観察O14と同じ族)。

**対策として不変条件をテストで縛った**:
上限の組み合わせ7通りを掃いて
**「すべての辺の source と target が nodes にある」**を検査する
(`test_neighborhood_edges_never_dangle`)。
**壊し確認: 修正前の順序に戻すと7通りのうち3通りが落ちる**(実際に確認)。
併せて「どの組み合わせでも辺が0本でないこと」も別テストで固定した
(空虚化の防止)。

### 実データの記録を訂正する

**観察B74と `docs/measurements-phase1.md` §20 に記録した辺の数は、
壊れた応答の数字だった。**

| 中心=厚生労働省・深さ2 | 修正前(記録した値) | 修正後(正しい値) |
|---|---|---|
| ノード | 100 | 100 |
| **エッジ** | **200** | **99** |
| 打ち切り | nodes・edges の両方 | **nodes のみ**(+分岐数5ノード) |

**200本のうち101本がダングリングだった。**
`edges_truncated` が真だったのも、**除外されたノードへの辺をエッジ数に
数えていたため**である —— 修正後は「返した nodes の間の辺は全部ある」という
意味を保つので、エッジ数の上限には達しない。

**所要時間の測定(0.453秒 / 1.125秒)は有効である**
(同じ数のクエリを流しているため)。**辺の数だけが誤っていた。**

### レビュアーの他の3点への回答(記録)

1. **双方向BFSの経路復元**: レビュアーが非対称グラフ複数で実行し、
   **反証は得られなかった**。`start == goal`・片側だけが伸びる場合・
   `len(edges) == len(nodes) - 1` の保持をいずれも確認。
2. **`exhaustive` の条件**: **反証を作れなかった**(5パターンで実行し、
   独立実装の素朴なBFSと突き合わせ)。紙上の理由も付いている ——
   `came[1-side]` は初期化時に相手側のルートを必ず含むため、
   片側が介入なしにフロンティアを使い果たした時点でその側の到達可能な
   全ノードは検査済みである。
   **効率面の指摘**: 片側が空のとき相手側に切り替えず全体を止めるため、
   非対称な場合に `visit_budget` を本来より多く消費しうる ——
   **正確性の問題ではないので今回は直さない**(記録する)。
3. **`_hydrate_graphs` の空Provenanceフォールバック**: 判断自体は妥当だが
   **この分岐は当初テストで一度も踏まれていなかった**(再発欠陥9)。
   **対応**: `Provenance.available: bool` を足し、引けなかったことを
   **明示する**ようにした —— 空文字列だけを返すのは
   **「出典が無い」ことを黙って隠す**ことであり、仕様§9.2
   「全表示要素に一次資料へのリンクと取得日時を出す」に対して
   UIが空のリンクを描く。**壊し確認の形のテストも追加した**
   (provenanceグラフから記述を外し、(a) 辺が消えない (b) `available=False`
   が立つ の両方を検査)。

### 実データで確認した(修正後)

ローカルのFuseki(884,052クアッド)に対して、上限が実際に効く5通りで
**ダングリングエッジ0件**を確認した:

```
{'depth': 1}                                          26ノード  25辺  0  fanout×1
{'depth': 2}                                         100ノード  99辺  0  nodes/fanout×5
{'depth': 2, 'node_limit': 10}                        10ノード   9辺  0  nodes/fanout×3
{'depth': 2, 'node_limit': 5, 'edge_limit': 50}        5ノード   4辺  0  nodes/fanout×1
{'depth': 2, 'node_limit': 50, 'edge_limit': 20, 'fanout_limit': 5}  21ノード 20辺 0  edges/fanout×4
```

### この裁定の位置づけ

**controllerが実装し、controllerが実データで検証し、それでも見落とした欠陥を、
第三者のレビュアーが実証した。** 裁定B72で「タスクレビューの席が空く」ことを
明示して繰り越した判断が、**実際に価値を生んだ** ——
繰り越さず「controllerの実データ検証で代替する」と決めていれば、
**ダングリングエッジはD-5の描画が壊れるまで見つからなかった。**

## 裁定 B97: 資金の流れを実データでKGに通し、CQ12が独立測定と一致した

**Dockerが停止しているためTDB2の索引は作れていない。** よって
`scripts/run_cq.py`(実エンドポイントでのCQ実行)は**走らせていない** ——
確認できたのは「オントロジーが実データに答えられること」であり、
**「Jena(TDB2)でも同じクエリが動くこと」は未確認**である(rdflibとJenaの
SPARQL実装は別物。run_cq.pyのdocstringが言う(b)が埋まっていない)。

### 経路

`parse_rs → build_projects → emit_budget → rdflibのDatasetにCQ12を実行`。
レイクは `project_summary`/`budget_summary`/`policy_measure…`が2026-08-23、
`payee_payment_information`/`payee_payment_block_connection`が2026-09-11
(5-1は両日で**sha256が完全一致** `e379dbc4aa4653b0…`・6,078,771バイト)。

### 規模

```
事業 5,794 / ブロック 20,482 / 間接経費 2,432
BudgetProject 5,794 / Expenditure 73,919
ExpenditureBlockRecord 20,482 / IndirectCostRecord 2,432
トリプル 956,661
```

ブロック20,482 = 5-1が金額を持つ19,125 + 5-2にしか現れない1,357。
後者は**捨てない** —— `budget:fundedBy` が存在しないブロックを指すと
参照整合ゲート(裁定B4)が落ちる(実装者の指摘)。

### CQ12の答え(**controllerの独立測定と完全一致**)

| | 金額 | 一致 |
|---|---:|:---:|
| CQ12(2025年度・14,587件) | **126,911,742,219,947 円** | ✅ |
| うち入口ブロック | 126,878,845,988,981 円 | ✅ |
| うち国自らが支出する間接経費 | 32,896,230,966 円 | ✅ |

**2つの独立な経路が同じ数字に着いた** —— controllerがCSVを直接走査した
ad-hocスクリプトと、コミットされた変換+出力+SPARQLの経路。

### 不変条件: 破れ0件

```sparql
?b a budget:ExpenditureBlock ; core:amount_jpy ?ba .
?e budget:inBlock ?b ; core:amount_jpy ?ea .
GROUP BY ?b ?ba  →  ?ba != SUM(?ea) の件数 = 0
```

**ブロックへの割り当てを誤れば崩れる検査であり、全ブロックで成立した。**
(ビルド時のゲートにも入れる価値がある。まだ入れていない)

### CQ13の答え(20行。`run_cq.py`は0件を失敗にする)

| 事業 | 段 | 金額 | 出どころ |
|---|---|---:|---|
| 物価高騰対応重点支援地方創生臨時交付金 | B「地方公共団体」 | 2,151,949,970,000 | A「総務省」 |
| 独立行政法人国際協力機構有償資金協力部門への出資 | E「開発途上地域の政府等」 | 1,826,971,153,000 | D「JICA」 |
| 同 | D「JICA」 | 1,826,971,152,858 | A「一般会計出資金」 |
| 同 | D「JICA」 | 1,826,971,152,858 | **B「財政融資資金借入金」** |
| 同 | D「JICA」 | 1,826,971,152,858 | C「回収金等」 |
| 障害者自立支援給付 | B「市区町村」 | 1,587,409,847,000 | A「都道府県」 |

**上位20件の通過金だけで 17,314,765,303,704 円** —— これが「足しては
いけない額」である。JICAの例は1つのブロック(D)が3つの出どころ(A/B/C)を
持ち、そのうちBが借入金(国の支出ではない)であることまで読める。

## 裁定 B99: 年度ごとの予算と執行を実データでKGに通した

**Dockerが停止しているためTDB2の索引は作れていない**(裁定B97と同じ制約)。
`parse_rs → build_projects → emit_budget → rdflibのDatasetにCQ14を実行`。

### 規模

```
事業 5,794 / AnnualBudgetLine 23,036 / AnnualBudgetRecord 23,036
(年度ごとの事業数: 2021:3,575 / 2022:3,807 / 2023:4,620 / 2024:5,240 / 2025:5,794)
```

### CQ14の答え(**controllerの独立測定とすべて一致**。恒等式も全年度で成立)

| シート | 予算年度 | 当初予算 | 歳出予算現額 | 執行額 | 事業数 | 恒等式 |
|---|---|---:|---:|---:|---:|:---:|
| 2025 | 2021 | 107,148,031,818,947 | 150,228,316,691,494 | 128,147,462,147,862 | 3,575 | ✅ |
| 2025 | 2022 | 108,070,350,924,489 | 143,762,728,268,781 | 120,356,191,402,344 | 3,807 | ✅ |
| 2025 | 2023 | 116,995,193,076,032 | 148,031,636,583,649 | 126,914,131,809,319 | 4,620 | ✅ |
| 2025 | 2024 | 121,113,149,838,813 | 147,625,005,687,732 | 128,407,860,724,145 | 5,240 | ✅ |
| 2025 | 2025 | 123,080,937,002,000 | 150,059,510,926,345 | **0**(未執行) | 5,794 | ✅ |

**シート年度が全行2025である**ことも確認した —— 5年分すべてが
「2025年度のレビューシートが記録していること」であり、年度ごとに別の
シートを取ってきて並べたものではない。

### nextYearRequest(controllerが「空」と誤認していた列)

| 予算年度 | 合計 | 件数 |
|---|---:|---:|
| 2021 | 111,305,746,476,457 | 3,575 |
| 2022 | 112,336,438,903,489 | 3,807 |
| 2023 | 122,132,200,692,046 | 4,620 |
| 2024 | 123,473,712,088,540 | 5,240 |
| 2025 | 126,353,016,606,456 | 5,794 |

**全件(23,036)が非空**。`'45013000.0'`という小数表記で、
`normalize_amount`が末尾の`.0`を落として取り込む。

### ビルド時の検査3本(実データ)

| 検査 | 破れ | 検査した件数 |
|---|---:|---:|
| 恒等式(当初+補正+繰越+予備費=現額) | **0** | 23,036 |
| `budgetAmount == シート年度のinitialBudget` | **0** | 5,794 |
| ブロックの金額 == 所属する支出の総和(B97) | **0** | 19,125 |

**壊し確認(実データ)**: 歳出予算現額を1円ずらす → 1件検出。
シート年度の当初予算を7円ずらす → 1件検出。

**controllerの検証スクリプト自身に2つの誤りがあった(記録)**:
(1) 検査の戻り値が `(破れ, 検査件数)` のタプルなのを `len()` で数え、
「破れ2件」と誤読した。(2) 当初予算をずらす壊し確認で、**シート年度以外
(2021年度)のレコード**を狙ったため検査が反応せず「捕まえられない」と
誤判定した。**どちらも「読まずに仮定した」ことが原因**で、
`.isdigit()` の件(裁定B99の訂正)と同じ族である。

### 要求と査定(nextYearRequest)

年度Yの`nextYearRequest`と年度Y+1の`initialBudget`の対応(両年度に存在する
事業に限る。`normalize_amount`で読んだ値):

| | 事業数 | 要求額 | 付いた当初予算 | 比 | 額が完全一致 |
|---|---:|---:|---:|---:|---:|
| 2021→2022 | 3,574 | 111,305,746,476,457 | 107,830,651,392,489 | 96.9% | 1,293 |
| 2022→2023 | 3,807 | 112,336,438,903,489 | 111,454,687,976,040 | 99.2% | 1,243 |
| 2023→2024 | 4,620 | 122,132,200,692,046 | 120,824,340,278,813 | 98.9% | 1,598 |
| 2024→2025 | 5,240 | 123,473,712,088,540 | 122,768,292,838,000 | 99.4% | 2,307 |

**全体では96.9〜99.4%が付くが、事業単位で額が完全一致するのは3〜4割。**
割合だけを見せると「査定が機能していない」という誤った印象を与えるので、
画面では完全一致の件数も並べて出す。

### 金額が負値を取る(申し送り。実装者の指摘)

集計行23,036件を検査(controllerと実装者が独立に測り、件数が一致):

| スロット | 負値 | 最小 |
|---|---:|---:|
| `supplementaryBudget` | 589 | -24,609,169,000円(事業648 FY2021) |
| `reserveFund` | 825 | **-518,000,000,000円**(事業2617 FY2021) |
| `carriedOverFromPreviousYear` | 1 | -3,197,015,000円(事業3788 FY2023) |
| `carriedOverToNextYear` | 1 | -3,197,015,000円(事業3788 FY2022) |
| `initialBudget` / `totalBudgetAvailable` / `executedAmount` / `nextYearRequest` | **0** | — |

減額補正・予備費の配分替えが負として現れる。**非負を仮定した集計・UI・
...
## 裁定B100/B101: 生成物と配信物の「同一性」を、プラットフォームとビルド環境で実測した(2026-09-11)

CIが2回落ちた。どちらも**対象ではなく、比較の作り方**が原因だった。
以下はその切り分けに使った実測。

### B100: スキーマ生成物はプラットフォームで並び順が変わる

| 測ったこと | 結果 |
|---|---|
| Windowsで生成した`.ttl`とCI(Linux)の再生成の差分 | **4ファイル・挿入150/削除150**(内容同一、順序のみ相違) |
| 入れ替わった対象 | 日本語の長い`sh:description`を持つプロパティ形状どうし(`sh:order`は持っている) |
| 生成物に含まれるCR | **0個**(「文字列リテラル中のCRが原因」という仮説を**否定**した) |
| Linuxコンテナでの再生成 | CIと**同じ4ファイル・同じ規模の差分**が出た(再現) |
| Linuxで2回連続の生成 | **バイト一致**(冪等。つまりハッシュ乱択などの非決定性ではない) |
| `frontend/src/generated/labels.json` | md5 `1d5edd913dbba6ffd67a4f2a42a1db9b`。Windows再生成後も同一(**プラットフォーム非依存**) |
| Linuxで作り直した4ファイルとCIの期待 | **一致**(コメント記載のdockerコマンドをそのまま実行して確認。2回実行して差分0) |

再現に使ったコマンド(`scripts/generate-schema.sh`のコメントにも置いた):
...
**`UV_PROJECT_ENVIRONMENT`を省くと、コンテナの`uv sync`がマウントした
`.venv`をLinux用に上書きしてホスト環境を壊す**(実際に踏んだ)。
`MSYS_NO_PATHCONV=1`を省くとGit Bashが`-w /w`を`W:/`に変換して落ちる。

### B101: フロントのバンドルは`VITE_API_BASE`でバイト列が変わる

| 測ったこと | 結果 |
|---|---|
| 同じコミットで2回ビルド(同一環境) | **同一ハッシュ** `index-Bwel7FMh.js`(ビルドは決定的) |
| `frontend/` の差分(本番配信元→HEAD) | **無変更** |
| `VITE_API_BASE` 無しでビルド | `index-Bwel7FMh.js` |
| `VITE_API_BASE` 付きでビルド | **`index-DfOLIwyo.js`** = 本番が配信しているファイル名 |
| そのファイルと本番配信物のsha256 | **一致** `61538c910271445c61f37e4d7d3dfe2bcf943c07fa4b380d42da7ac5cab1e0ee` |
| 本番バンドルに埋まっている文字列 | `https://jgkg.gentlemeadow-d9ba6656.japaneast.azurecontainerapps.io` |
| `vars.VITE_API_BASE` の設定日 | 2026-09-05(この日から日次検査が落ち始めた) |

**本番は最新だった。** 日次検査は`VITE_API_BASE`を渡さずにビルドした
成果物と本番を比べていた。

### 本番リリースとの差(参考。手元のmanifest比較)

| 項目 | 旧(2026-08-28) | 新(2026-09-11) |
|---|---:|---:|
| triple_count | 884,052 | **1,438,620**(+554,568) |
| tdb2展開後 | 466,005,179 B | 679,029,760 B |
| 名前付きグラフ | `rs-system/2026-08-23` | `rs-system/2026-09-11` に差し替え |
| 他4ソースの取得日 | 変化なし | 変化なし |
| 配信イメージ | 1.08GB | **1.34GB** |

**↑ この節はもともと「まだ本番に載っていない。外向きの操作なので
利用者の意図表明を待つ」と書いていた。2026-09-11に利用者の承認を得て
載せた** —— GitHub Releasesでの公開と本番ACAの配信イメージの両方。
配備は`provisioningState: Succeeded`まで確認した(裁定B101/B102)。
現状は`docs/status.md`と`docs/deploy-aca.md`の2026-09-11の節を参照。

### 配備の前後で測ったこと(2026-09-11)

| 測ったこと | 結果 |
|---|---|
| イメージ内manifestの宣言 vs Fusekiへの`COUNT(*)` | どちらも **1,438,620**(一致) |
| `smoke-test-api.py`(ローカル・本番) | 両方で5経路通過 |
| CQ12(ローカルの配信イメージ) | **126,911,742,219,947円**(記録済みの値と一致) |
| 本番`/entity/`で新クラス | `ExpenditureBlock`(`type`・`label`・出典付き)・`AnnualBudget` 両方引けた |
