...
| B-4 | **abr-geocoder(§8.1)は、RSの支出先に住所列がある場合のみ導入する**(Task 6 の分岐)。住所列が無ければ照合材料が存在しないため導入しない | §7.3 と同じ「実データで確認して分岐」の型 |
...
- [ ] **Step 3: 名称正規化を実装**(§8.1 の2段目。法人種別語(株式会社/(株)/㈱)の
  ...
  全載せしない**。R19 と同じ理由)
- [ ] **Step 4: 経路2 — basis_law_text から法令を引く**(law_title / abbrev の
  **完全一致のみ**。「〜法第3条」のような後置は法令名部分の前方一致で1件に
  絞れる場合のみ解決。解決率を数え、未解決は件数だけ報告 — エッジは張らない。
  §7.2「精度目標であり完了条件でない」)
- [ ] **Step 5: emit_budget**(名前付きグラフ `graph/rs-system/{取得日}`。
  出典に Task 1 の sha256。閉じたシェイプで全域が通ることを SHACL テストで確認)
- [ ] **Step 6: 全テスト → 3回バイト一致 → コミット**
