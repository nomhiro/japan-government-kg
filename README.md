...
> [!NOTE]
> **現在の状態: Phase 0(完了条件9件)・Phase 1(完了条件3件)をいずれも実データで完了。テスト632件。**
...
>
> 詳細は **[進捗状況](docs/status.md)** を参照(Phase 0・Phase 1 と、その後の配布・自動化まで反映)。
...
- [**実装決定記録**](docs/decision-log.md) — 実装中に下した裁定B1〜B54とその理由。実データに当てて初めて分かった落とし穴と、覆した判断も含む(自分の誤りもそのまま残している)
